...
import com.example.cash_you.data.DatabaseManager;
import com.example.cash_you.data.PersonManager;
import com.example.cash_you.models.*;
...
class CategoryColors {
    private static HashMap<String, Integer> categoryColors = new HashMap<>();
...
class Tab {
    ...
    //Add transaction
    private void AddTransaction(MyTabTransaction tran) {
        if (tran != null) {
            _transactions.add(tran);
        }
    }
    private void AddTransactions(List<MyTabTransaction> trans) {
    ...
    private void DeleteTransaction(MyTabTransaction tran) {
        if (tran != null) {
    ...
    private MyTabTransaction GetTransactionByID(String id) {
        for (int i = 0; i < _transactions.size(); i++) {
            if (_transactions.get(i).toString().equals(id)) {
    ...
    private int GetTransactionIndex(MyTabTransaction transaction) {
    ...
    private void SetPersonA(String name) { _personA = name; }
    private void SetPersonB(String name) { _personB = name; }
    private void SetPaid(boolean paid) { _paid = paid; }
    private void SetDateStart(Calendar start) {
        if (_end == null || start != null && start.before(_end)) {
            _start = start;
        }
        else if (start == null) { _start = null; }
    }
    private void SetDateEnd(Calendar end) {
        if (end != null && end.after(_start)) { _end = end; }
        else if (end == null) { _end = null; }
    }
    private void SetPaidDate(Calendar paidDate) { _paidDate = paidDate; }
